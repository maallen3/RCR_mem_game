var mem = {
  ...
  // (A2) GAME SETTINGS
  sets : 12, // Total number of cards to match
    ...
    if (mem.last == null) { mem.last = idx;var d1 = new Date();mem.time1 = d1.getTime();}
      ...
      if (mem.grid[idx] == mem.grid[mem.last]) {
        ...
        if (mem.matches.length == mem.sets * 2) {
          var d2 = new Date();
          mem.time2 = d2.getTime();
          var diff = Math.abs(mem.time2-mem.time1)
          alert("YOU WIN! TOTAL MOVES " + mem.moves+"! It took you "+diff+ " milliseconds.");
          mem.init();
        }
      }
